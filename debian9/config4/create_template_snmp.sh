...
create_cmd_snmp() {
  ...
  [ $? -ne 0 ] && $CLAPI -o CMD -a ADD -v 'cmd_os_linux_snmp_load;check;$CENTREONPLUGINS$/centreon_linux_snmp.pl --plugin=os::linux::snmp::plugin --mode=load --warning=$_SERVICEWARNING$ --critical=$_SERVICECRITICAL$ --host=$HOSTADDRESS$ --snmp-version=$_HOSTSNMPVERSION$ --snmp-community=$_HOSTSNMPCOMMUNITY$ $_HOSTOPTION$ $_SERVICEOPTION$'

  # check_centreon_plugin_cpu_SNMP
  exist_object CMD cmd_os_linux_snmp_cpu 
  [ $? -ne 0 ] && $CLAPI -o CMD -a ADD -v 'cmd_os_linux_snmp_cpu;check;$CENTREONPLUGINS$/centreon_linux_snmp.pl --plugin=os::linux::snmp::plugin --mode=cpu --warning-average=$_SERVICEWARNINGAVERAGE$ --critical-average=$_SERVICECRITICALAVERAGE$ --warning-core=$_SERVICEWARNINGCORE$ --critical-core=$_SERVICECRITICALCORE$ --host=$HOSTADDRESS$ --snmp-version=$_HOSTSNMPVERSION$ --snmp-community=$_HOSTSNMPCOMMUNITY$ $_HOSTOPTION$ $_SERVICEOPTION$'

  # check_centreon_plugin_memory_SNMP
  exist_object CMD cmd_os_linux_snmp_memory
  [ $? -ne 0 ] && $CLAPI -o CMD -a ADD -v 'cmd_os_linux_snmp_memory;check;$CENTREONPLUGINS$/centreon_linux_snmp.pl --plugin=os::linux::snmp::plugin --mode=memory --warning-usage=$_SERVICEWARNING$ --critical-usage=$_SERVICECRITICAL$ --host=$HOSTADDRESS$ --snmp-version=$_HOSTSNMPVERSION$ --snmp-community=$_HOSTSNMPCOMMUNITY$ $_HOSTOPTION$ $_SERVICEOPTION$'


  #check_centreon_plugin_SNMP_traffic
  exist_object CMD cmd_os_linux_snmp_traffic  
  [ $? -ne 0 ] && $CLAPI -o CMD -a ADD -v 'cmd_os_linux_snmp_traffic;check;$CENTREONPLUGINS$/centreon_linux_snmp.pl --plugin=os::linux::snmp::plugin --mode=interface --speed-in=$_SERVICESPEEDIN$ --speed-out=$_SERVICESPEEDOUT$ --interface=$_SERVICEINTERFACE$ --warning-in-traffic=$_SERVICEWARNINGIN$ --critical-in-traffic=$_SERVICECRITICALIN$ --warning-out-traffic=$_SERVICEWARNINGOUT$ --critical-out-traffic=$_SERVICECRITICALOUT$ --host=$HOSTADDRESS$ --snmp-version=$_HOSTSNMPVERSION$ --snmp-community=$_HOSTSNMPCOMMUNITY$ $_SERVICEOPTION$'

  #check processcount 
  exist_object CMD cmd_os_linux_snmp_process
  [ $? -ne 0 ] && $CLAPI -o CMD -a ADD -v 'cmd_os_linux_snmp_process;check;$CENTREONPLUGINS$/centreon_linux_snmp.pl --plugin=os::linux::snmp::plugin --mode=processcount --hostname=$HOSTADDRESS$ --snmp-version=$_HOSTSNMPVERSION$ --snmp-community=$_HOSTSNMPCOMMUNITY$ $_HOSTOPTION$ --process-name=$_SERVICEPROCESSNAME$ --process-path=$_SERVICEPROCESSPATH$ --process-args=$_SERVICEPROCESSARGS$ --regexp-name --regexp-path --regexp-args --warning=$_SERVICEWARNING$ --critical=$_SERVICECRITICAL$ $_SERVICEOPTION$' 


  # cmd_os_linux_local_disk_name
  exist_object CMD cmd_os_linux_snmp_disk_name
  [ $? -ne 0 ] && $CLAPI -o CMD -a ADD -v 'cmd_os_linux_snmp_disk_name;check;$CENTREONPLUGINS$/centreon_linux_snmp.pl --plugin=os::linux::snmp::plugin --mode=storage --hostname=$HOSTADDRESS$ --snmp-version=$_HOSTSNMPVERSION$ --snmp-community=$_HOSTSNMPCOMMUNITY$ $_HOSTOPTION$ --name=$_SERVICEDISKNAME$ --warning-usage=$_SERVICEWARNING$ --critical-usage=$_SERVICECRITICAL$ $_SERVICEOPTION$ '
