...
create_cmd_snmp() {
  ...
  [ $? -ne 0 ] && exec_clapi CMD ADD 'cmd_os_linux_snmp_cpu;check;$CENTREONPLUGINS$/centreon_linux_snmp.pl --plugin=os::linux::snmp::plugin --mode=cpu --warning-average=$_SERVICEWARNING$ --critical-average=$_SERVICECRITICAL$ --host=$HOSTADDRESS$ --snmp-version=$_HOSTSNMPVERSION$ --snmp-community=$_HOSTSNMPCOMMUNITY$ $_HOSTOPTION$ $_SERVICEOPTION$'
