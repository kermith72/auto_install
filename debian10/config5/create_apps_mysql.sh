...
create_stpl_mysql() {
  ...
  then
    ...
    exec_clapi STPL setparam "stpl_app_db_mysql-databases-size;check_command;cmd_app_db_mysl_databases-size"
    exec_clapi STPL setmacro "stpl_app_db_mysql-databases-size;filterdatabase;'^(?!(information_schema|performance_schema|test))'"
    exec_clapi STPL setmacro "stpl_app_db_mysql-databases-size;filterperfdata;'database'"
  fi
