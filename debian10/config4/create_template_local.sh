...
create_cmd_local() {
  ...
  [ $? -ne 0 ] && exec_clapi CMD ADD 'cmd_os_linux_local_swap;check;$CENTREONPLUGINS$/centreon_linux_local.pl --plugin=os::linux::local::plugin --mode=swap --warning=$_SERVICEWARNING$ --critical=$_SERVICECRITICAL$ $_SERVICEOPTION$ '
